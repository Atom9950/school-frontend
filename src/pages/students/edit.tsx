...
// Create a modified schema for edit mode where department is optional
const studentEditSchema = studentSchema.omit({ department: true }).extend({
    ...
    useEffect(() => {
        if (student) {
            reset({
                ...
                joiningDate: student.admissionDate ? new Date(student.admissionDate).toISOString().split('T')[0] : '',
                department: student.department?.id || 0,
                bannerUrl: student.image || '',
                bannerCldPubId: student.imageCldPubId || '',
            });
        }
    }, [student, reset]);

    const onSubmit = async (values: z.infer<typeof studentEditSchema>) => {
        try {
            // Use the student's current department since the field is disabled
            const departmentId = student?.department?.id || values.department;
    ...
    if (query.isLoading) {
        return (
            <CreateView className="student-view">
                ...
                <p className="state-message">Loading student details...</p>
            </CreateView>
        );
    }
    ...
    if (query.isError || !student) {
        return (
            <CreateView className="student-view">
                <Breadcrumb />
                <h1 className="page-title">Edit Student</h1>
                <p className="state-message">Error loading student details</p>
            </CreateView>
        );
    }
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormDescription>
                                                    Department cannot be edited. Use Promote or Transfer buttons to change department.
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
